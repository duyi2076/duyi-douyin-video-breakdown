...
- 只有 CLI 没有 Browser Bridge 时，AI 必须把 [`references/opencli-setup.md`](references/opencli-setup.md) 中的连接步骤直接告诉用户；连续两次 `doctor` 失败时，若具备浏览器截图能力，优先用截图标注安装步骤。不要只报错，也不要假装页面采集成功。用户仍未连接时，改用本地视频，或明确报告链接采集被浏览器连接阻塞。
